...
Item {
  ...
  function cameraEnabled(){return renderer&&renderer.prop("camera_enabled",true)!==false}
  function audioInputEnabled(){return renderer&&renderer.prop("audio_input_enabled",true)!==false}
  ...
  AudioInput{id:nativeAudioInput;device:sessionRoot.device(nativeDevices.audioInputs,renderer?renderer.prop("audio_input_device",""):"",nativeDevices.defaultAudioInput);muted:!sessionRoot.audioInputEnabled()||!sessionRoot.deviceValid(nativeDevices.audioInputs,renderer?renderer.prop("audio_input_device",""):"","audio_input")}
