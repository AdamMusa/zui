...
// Qt 6 / QSB port for Omarchy UI.
// The original Shadertoy source and its CC0 attribution are retained below.
...
#define TAU             (2.0*PI)
#define TIME            (time * max(0.15, frequency * 0.32))
...
void main() {
  ...
  vec4 rendered;
  mainImage(rendered, fragCoord);
  rendered.rgb *= max(intensity, 0.0);
  qt_FragColor = rendered * qt_Opacity;
}
